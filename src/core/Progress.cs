...
public sealed class Progress
{
    ...
    public int GetWrongCount(string question) =>
        _wrongCounts.TryGetValue(question, out var n) ? n : 0;

    public void AddWrong(string question) =>
        _wrongCounts[question] = GetWrongCount(question) + 1;

    public string ToJson() =>
        JsonSerializer.Serialize(_wrongCounts, new JsonSerializerOptions { WriteIndented = true });
