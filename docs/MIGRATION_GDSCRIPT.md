...
여기서 **마이그레이션**은 적절한 표현이다. 기존 기능, 씬, 데이터 형식과 설계 의도를 유지한 채 구현 언어와 실행 기반을 바꾸기 때문이다.
...
- C# 실행 환경은 새로 설정하지 않는다.
- 따라서 C# 앱 실행 결과나 C# 테스트 통과 결과를 기준선으로 요구하지 않는다.
- 현재 `master`의 C# 소스, 씬, 테스트 코드, 문서를 **읽을 수 있는 동작 명세**로 삼는다.
- GDScript 구현부터 일반 Godot에서 실행·테스트한다.
- C#과 GDScript를 동시에 실행해 결과를 비교하는 검증은 수행하지 않는다.

이 선택의 대가는 기존 C# 실행 화면과 픽셀 단위로 비교할 수 없다는 점이다. 대신 기존 코드의 조건문·테스트 이름·입출력 사례를 기능 체크리스트로 바꾸고, 새 구현은 그 체크리스트와 수동 UX 검증을 통과시킨다.
...
GDScript 버전은 기존 C# 버전이 만들었을 수 있는 파일을 읽을 수 있어야 한다. C# 실행 환경을 준비하지 않으므로, 역방향 호환은 테스트 요구사항으로 두지 않는다.
...
Phase 0의 목적은 코드를 옮기는 것이 아니라, 이후 작업이 확인할 **고정 입력과 실행 출발점**을 만드는 것이다. 이 단계에서는 C#을 실행하지 않는다.

#### 0-A. 저장소에서 지금 할 일

- 마이그레이션 기준 커밋을 `5110604` (`2026-07-23`, 학습 인터랙션)으로 기록한다.
- `tests/fixtures/`에 아래 여섯 fixture를 추가한다.
  - 빈 덱
  - 여러 줄 답과 빈 답이 있는 덱
  - 중복 질문이 있는 덱
  - 예전 진행도 JSON
  - 현재 진행도 JSON
  - 깨진 진행도 JSON
- 각 fixture의 기대 결과를 `tests/fixtures/README.md`에 적는다.

**완료 기준:** 실제 사용자 데이터와 분리된, 반복해서 읽을 수 있는 테스트 입력이 저장소에 있다.

#### 0-B. 일반 Godot을 설치한 직후 할 일

- Godot Project Manager에서 이 프로젝트 폴더를 Import한다.
- 일반 Godot 에디터로 프로젝트가 열리는지 확인한다. 이 시점에는 C# 씬 오류가 나도 정상이다.
- 새 빈 GDScript 테스트 씬을 실행해 Output에 `Phase 0 ready`를 출력한다.
- 실행 창에서 `res://`와 `user://` 경로를 출력해 확인한다.

**완료 기준:** 일반 Godot에서 GDScript 씬 하나를 실행할 수 있다.

#### 이 단계에서 배우는 것

- Project Manager는 프로젝트를 생성·Import·선택해 에디터를 여는 시작 창이다.
- `res://`는 프로젝트 안의 읽기 전용 리소스 경로이고, `user://`는 앱이 저장하는 사용자 데이터 경로다.
- Project Settings의 Main Scene은 F6가 아니라 F5로 실행할 대상을 정한다.

#### 다음 단계로 갈 조건

0-A와 0-B가 끝나면 Phase 1에서 `FlashCard`와 `DeckNaming`을 GDScript로 구현한다. 일반 Godot이 아직 없다면 0-A까지만 완료로 기록하고 설치 뒤 0-B를 한다.
