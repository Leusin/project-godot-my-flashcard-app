...
- 진행도 JSON은 **현재 Question**을 키로 저장한다 (파일은 계속 사람이 읽을 수 있다).
- 카드는 편집 세션 동안 자기 정체성을 유지한다 (런타임 ID). 앱에서 질문을 `A→B`로 고치면, 화면을 나갈 때 진행도가 그 카드를 따라 `A→B`로 옮겨진다 — **초기화되지 않는다.** 세션이 "이 카드가 A였다"를 알기 때문에 가능하다.
- 추가·삭제·순서 변경도 같은 원리로 진행도가 카드를 따라간다.
- **한계**: 앱 밖(PC)에서 `.md`의 질문을 직접 고치면 매핑을 아는 세션이 없어 그 카드의 진행도는 끊긴다. 이 경우는 커버하지 않는다 (마크다운에 기계용 ID를 심지 않기 위한 의도적 타협).
- 이사한 키가 이미 있는 질문과 겹치면(같은 덱 내 중복 질문) 어떻게 합칠지는 편집기 구현 시 정한다.
