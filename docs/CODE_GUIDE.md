...
| 4 | `src/core/Progress.cs` · `CardStatus.cs` | Question별 상태(틀린 횟수 + 표시용 라벨) + JSON. `Rename`/`Remove`로 이사·정리. 옛 형식(숫자)도 읽음 |
...
| 13 | `screens/card_list/card_list.tscn` · `CardListView.cs` | 한 덱의 카드 목록(질문 + 틀린 횟수). 고른 카드·추가·내보내기·뒤로를 시그널 |
| 14 | `screens/card_editor/card_editor.tscn` · `CardEditorView.cs` | 질문/답 + 틀린 횟수·상태 편집. 나갈 때 입력값·삭제를 사실로 알림 (저장·삭제 해석은 App) |
