...
| 5 | `src/core/DeckNaming.cs` · `DeckInfo.cs` · `AppSettings.cs` | 덱 파일 이름 규칙, 목록 한 줄의 사실, 마지막 사용 덱 |
...
| 13 | **`src/app.tscn` · `src/App.cs`** | **진입점: 화면 전환(목록 ↔ Study), Import, 마지막 사용 덱 기억** |
| 14 | `tests/TestRunner.cs` | 위 전부의 테스트 |

`Study`는 어떤 덱을 열지 스스로 정하지 않는다. `App`이 `StartDeck(deckFile)`로 지정하고, `Study`는
화면을 떠나고 싶다는 사실(`ExitRequested`)만 위로 알린다.
...
```
                App  ── 마지막 덱 기억, 화면 전환, Import
       ┌─────────┴─────────┐        ↑ 위로는 "사실"만 올라간다
       ↓                   ↓
  DeckListView          Study  ── 세션 규칙, 진행도
   "고른 덱: X"       "나가고 싶다"
   "이 파일 가져와"
```
...
- **아래는 위를 모른다.** `DeckListView`는 App도 Study도 모르고, 어떤 덱을 골랐다는 사실만 시그널로 올린다. `Study`도 다음 화면이 무엇인지 모르고 `ExitRequested`만 올린다.
- **그래서 화면을 추가할 때 고칠 곳은 App 하나다.** 새 화면은 자기 시그널만 내면 된다.
- 화면끼리는 서로를 모른다. 목록 → Study 전환은 App의 해석이다.

## 앱 저장소 (`user://`)

md 원본은 앱 저장소 안에 있다. 가져오기(Import)는 파일을 이쪽으로 복사하고, 이후로는 이쪽이 원본이다.

```
user://decks/<덱이름>.md        가져온 덱. 목록에 나오는 것은 여기 있는 파일뿐
user://progress/<덱이름>.json   진행도. 덱마다 따로 (카드 식별자가 Question이라 덱 간 충돌을 막는다)
user://settings.json            마지막 사용 덱
```

`res://sample_deck.md`는 덱이 하나도 없을 때만 `user://decks`로 복사되는 예시다. 앱이 직접 읽지 않는다.
...
src/screens/  화면 한 벌씩 (씬 + 스크립트를 같은 폴더에)
