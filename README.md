...
Markdown으로 카드를 작성하고, 스와이프와 버튼으로 복습하는 모바일 중심 플래시카드 앱. 첫 Google Play 테스트 배포를 준비 중이다 (버전 0.9.0, 패키지 `com.leusin.mysimpleflashcard`).
...
| Android Release AAB | Play 업로드용 release AAB, target SDK 36 | `build/android/my-simple-flash-card-0.9.0.aab` |

- 앱 이름 **My Simple Flash Card**, 패키지 `com.leusin.mysimpleflashcard`, versionName `0.9.0`, versionCode `1`.
...
& $godot --headless --path . --export-release "Android Release AAB" build/android/my-simple-flash-card-0.9.0.aab
```
